...
envall=dat %>% 
  select( vache_std, conductivite:temperature,
          aire, profondeur, 
          invertebres:poissons) %>%
  subset(oxygene>0) %>% 
  decostand("standardize") 
envall=cbind(subset(dat, oxygene>0)$cloture, envall) %>% 
  rename(cloture= `subset(dat, oxygene > 0)$cloture`)
rownames(envall) <- subset(dat, oxygene>0)$etang
...
env=dat %>% 
  ...
  subset(oxygene>0) %>% 
  decostand("standardize") 
env=cbind(subset(dat, oxygene>0)$cloture, env) %>% 
  rename(cloture= `subset(dat, oxygene > 0)$cloture`)
rownames(env) <- subset(dat, oxygene>0)$etang

first=rda(amphi.hel~ . , envall)
second=rda(amphi.hel~ . + Condition(topo$aire +topo$profondeur), env) # add condition (topo)
...
anova(first, permutations = how(nperm = 999))
anova(first, by="axis", permutations = how(nperm = 999))
anova(first, by="term", permutations = how(nperm = 999))

anova(second, permutations = how(nperm = 999))
anova(second, by="axis", permutations = how(nperm = 999))
anova(second, by="term", permutations = how(nperm = 999))
vif.cca(second)

par(mfrow=c(1,2))
triplot.rda(second, scaling=1, 
            move.origin=c(0, 0), 
            mar.percent=0.05,
            silent=T)
triplot.rda(second, scaling=2, 
            move.origin=c(0, 0), 
            mar.percent=0.05,
            silent=T)
...
amphi.hel<-decostand(amphi.pa, "hellinger")
head(amphi.och)
