...
raw_data=data.frame(bday=c(rnorm(n=10, mean=20, sd=5),
                         rnorm(n=10, mean=10, sd=4),
                         rnorm(n=10, mean=5, sd=3),
                         rnorm(n=10, mean=4, sd=2),
                         rnorm(n=10, mean=3, sd=1),
                         rnorm(n=10, mean=2, sd=0.5)),
                    poptot=sort(rep(1:6, 10)))

dumb_data=raw_data %>% group_by(poptot) %>% 
  summarise(fit=mean(bday),
            sdfit=sd(bday))


ggplot(raw_data)+
  geom_boxplot(aes(x=factor(poptot), y=bday))+
  geom_ribbon(data=dumb_data, aes(x=poptot, ymin=fit-sdfit, ymax=fit+sdfit), alpha=.5)+
  geom_line(data=dumb_data, aes(x=poptot, y=fit), lwd=2)+
  xlab("une belle variable vraiment continue")+
  ylab("l'affaire que\nt'essaies de modéliser")+
  ggtitle("Un graphique arrangé\na'c le gars des vues")+
  theme_bw()
...
Dans notre cas, on ne s'attend pas à ce que la variance des Ys soit homogène (on ne fait pas des mod`les linaéres où les résidus devraient être homogènes, etc.); qui plus est, ce n'est pas une supposition à respecter dans le cadre d'une analyse multivariée...
