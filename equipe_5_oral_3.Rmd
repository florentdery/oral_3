...
ggplot(sit.pres)+
  ...
  scale_y_continuous("Richesse spécifique",
                     breaks= 1:6) +
    ggtitle("Selon l'ordre des données")+
  theme_classic() +
...
ggplot()+
  ...
  scale_y_continuous("",
                     breaks= 1:6) +
  ggtitle("Triés selon leur profondeur")+
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90)),
...
- Les variables Y sont numériques continues, distribuées dans un intervalle allant de 0 à 0,86 (la somme pour chaque espèce est égale à 1). 
- Il y a énormément de zéros, près de 71% des données de la matrice sont égales à 0, ce qui est un problème pour les données de communautés écologiques. 

## Choix de l'analyse (étape 7)

Nous avons décidé d'utiliser une analyse de redondance partielle pour prendre en compte l'effet de l'aire et de la profondeur des étangs.

### Choix de transformation de la matrice d'abondance: 
...
Il faut encore une fois transformer les données à l'aide d'un coefficient binaire assymétrique. 
...
amphi.hel= dat%>% 
    subset(oxygene>0) %>% # on enlève les sites avec l'oxygène à 0
  select(A_Gr:N_Vi) %>% 
  decostand("hellinger")

rownames(amphi.hel) <- subset(dat, oxygene>0)$etang# On change le nom des lignes pour se retrouver plus tard
...
env=cbind(subset(dat, oxygene>0)$cloture, 
          env) %>% # on rattache la variable cloture, qui n'est pas standardisée
  rename(cloture= `subset(dat, oxygene > 0)$cloture`)
