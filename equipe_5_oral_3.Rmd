...
Les données d'abondances sont loins d'être *Guaussienne*. Les RDAs sont des tests non-paramétriques, donc elles conviennent très bien aux données d'abondance que nous avons. À l'inverse, une MANOVA exige une distribution normalke multivariée. 
...
```{r RDA_code, message=FALSE, eval=F}
...
Test de permutation du modèle global: 
```{r RDA_check_model}
#source("panelutils.R")
anova(prda, permutations = how(nperm = 999))
```
Ceci nous indique que le modèle a quand même une certaine valeur, il permet d'expliquer une proportion de la variance dans les communautés d'abondances


## Étape 8.2 Valider l'analyse 

Test de permutations sur les axes:
```{r RDA_check_axis}
anova(prda, by="axis", permutations = how(nperm = 999))
```
Seulement le premier axe de la RDA est *significatif*


## Étape 8.2 Valider l'analyse 

Tests de permutations sur les variables explicatives

```{r RDA_checks_terms}
anova(prda, by="term", permutations = how(nperm = 999))
```

Les variables clôture, nitrates, temperature et poissons sont celles qui semblent mieux expliquer la variance dans la matrice d'abondances des amphibiens. Il pourrait être envisageable de faire une sélection forward des variables pour réduire le nombre de variables dans la RDA et ainsi mieux obtenir la proportion de variance influencée par les variables explicatives pertinentes.  
...
On a voulu vérifier si la transformation des variables environnementales influençait les résultats de la RDA. 
option 1, on log des variables avant de les transformer

```{r transform_n_standardize_variables, results="hide"}
topo=dat %>% 
    subset(oxygene>0) %>% 
  select(aire, profondeur) %>% 
  decostand("standardize")
...
#Donc les valeurs de temperatures élevées sont dans les étangs les plus petits et moins profonds. Rassurant, et ce ne sont probablement pas des valeurs erronées.  
# Ces deux étangs ont aussi des valeurs dans  le 1er quantile et des concentrations d'oxygène dans le 3e quantile.
...
dat %>% 
  subset(pH<4|pH>9) %>% 
  select(etang, pH, invertebres:poissons) # effectivement, dur de dire ce qu'il s'est passé, mais même avec un pH de 1.38, des poissons ont été comptés dans l'étang  # 155

# Tranformation ####

# Jetter un coup d'oeil aux options de transformation pour la température, les poissons et les écrevisses.
...
hist(((dat$temperature^lambda)-1)/lambda , 
     main="temperature box-cox(lambda=-2)")
...
env2=cbind(subset(dat, oxygene>0)$cloture, 
          env2) %>% 
  # on rattache la variable cloture, qui n'est pas standardisée (catégorique)^^
  rename(cloture= `subset(dat, oxygene > 0)$cloture`)+ # un nom qui a de l'allure.
  mutate(poiss_bin=ifelse(poissons>0, "Yes", "No"),
         ecre_bin=ifelse(ecrevisses>0, "Yes", "No")) %>% 
  select(-poissons, -ecrevisses)
...
require(vegan)
(prda2=rda(amphi.hel~ . + Condition(topo$aire +topo$profondeur), env) )
```
