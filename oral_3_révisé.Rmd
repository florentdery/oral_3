...
## 8.3 - Interpréter les résultats : triplots À FINIR AVEC SUBSET DE VARIABLES ENVIRO
...
```{r var-par-modele-global, echo=F, message=F, warnings=F, results="hide", fig.dim=c(6.5, 5.3), fig.align="left"}
...
```{r trnd_RDA_data_prep, message=FALSE}
topo=dat %>% 
    subset(oxygene>0) %>% 
  select(aire, profondeur) %>% 
  decostand("standardize")
...
```{r trnd_RDA_CHECKS, message=FALSE}
set.seed(1)
...
et le partitionnement de la variance pour ce modèle : 
